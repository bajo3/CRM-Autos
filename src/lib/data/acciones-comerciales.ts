...
export async function getAccionesComerciales(): Promise<AccionItem[]> {
  ...
  const [segRes, presRes, credRes, resRes, encRes, postRes, tdRes, cumpleRes] = await Promise.all([
    sb.from("seguimiento")
      .select("id,fecha,motivo,estado,cliente:cliente_id(id,nombre,apellido,telefono,whatsapp)")
      .in("estado", ["pendiente", "vencido"]).lte("fecha", today)
      .order("fecha").returns<SeguimientoRow[]>(),
    sb.from("presupuesto")
      .select("id,validez,precio,estado,cliente:cliente_id(id,nombre,apellido,telefono,whatsapp),vehiculo:vehiculo_id(marca,modelo)")
      .eq("estado", "enviado").not("validez", "is", null).lte("validez", in3days)
      .order("validez").returns<PresupuestoRow[]>(),
    sb.from("credito")
      .select("id,cuota_actual,cantidad_cuotas,estado,venta:venta_id(cliente:cliente_id(id,nombre,apellido,telefono,whatsapp),vehiculo:vehiculo_id(marca,modelo))")
      .in("estado", ["activo", "por_terminar"])
      .returns<CreditoRow[]>(),
    sb.from("reserva")
      .select("id,vencimiento,monto_sena,estado,cliente:cliente_id(id,nombre,apellido,telefono,whatsapp),vehiculo:vehiculo_id(marca,modelo)")
      .in("estado", ["activa", "vencida"])
      .order("vencimiento").returns<ReservaRow[]>(),
    sb.from("encargo")
      .select("id,marca_buscada,modelo_buscado,urgencia,estado,cliente:cliente_id(id,nombre,apellido,telefono,whatsapp)")
      .in("estado", ["buscando", "unidad_encontrada", "ofrecido"]).eq("urgencia", "alta")
      .returns<EncargoRow[]>(),
    sb.from("postventa")
      .select("id,fecha_alerta,realizada,cliente:cliente_id(id,nombre,apellido,telefono,whatsapp)")
      .eq("realizada", false).lte("fecha_alerta", in3days)
      .order("fecha_alerta").returns<PostventaRow[]>(),
    sb.from("test_drive")
      .select("id,fecha,hora,conductor_nombre,telefono,cliente:cliente_id(id,nombre,apellido,telefono,whatsapp),vehiculo:vehiculo_id(marca,modelo)")
  ...
  for (const s of segRes.data ?? []) {
    ...
    items.push({
      ...
      cliente: nombreCompleto(c), clienteId: c?.id ?? null, telefono: tel(c),
  ...
  for (const t of tdRes.data ?? []) {
    ...
    items.push({
      ...
      cliente: nombreConductor ?? "Conductor sin nombre", clienteId: c?.id ?? null, telefono: t.telefono || tel(c),
  ...
  for (const cl of cumpleRes.data ?? []) {
    ...
    items.push({
      ...
      cliente: nombre, clienteId: cl.id, telefono: tel(cl),
