...
export async function createCredit(payload: Omit<CreditRow, "id" | "created_at" | "updated_at" | "closed_at" | "status"> & { status?: CreditStatus }) {
  ...
  void logCrmEvent({ entity_type: "credit", entity_id: row.id, type: "credit_created", payload: { client_name: row.client_name } });
  return row;
}
...
export async function updateCredit(id: string, patch: Partial<CreditRow>) {
  ...
  const row = data as CreditRow;
  void logCrmEvent({ entity_type: "credit", entity_id: id, type: "credit_updated", payload: { patch } });
  return row;
}

export async function closeCredit(id: string) {
  const row = await updateCredit(id, { status: "closed", closed_at: new Date().toISOString() } as any);
  void logCrmEvent({ entity_type: "credit", entity_id: id, type: "credit_closed" });
  return row;
}
