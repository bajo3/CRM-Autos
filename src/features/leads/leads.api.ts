...
export async function createLead(input: LeadInsert) {
  ...
  await logCrmEvent({ entity_type: "lead", entity_id: created.id, type: "lead_created", payload: { name: created.name, stage: created.stage } });
  return created;
}
...
export async function updateLead(id: string, patch: LeadUpdate, opts?: { skipEvent?: boolean }) {
  ...
  if (!opts?.skipEvent) {
    if (patch.stage) {
      ...
      await logCrmEvent({ entity_type: "lead", entity_id: id, type: "lead_stage", payload: { stage: patch.stage } });
      ...
      await logCrmEvent({ entity_type: "lead", entity_id: id, type: "lead_assigned", payload: { assigned_to: (patch as any).assigned_to ?? null } });
    } else {
      await logLeadEvent(id, "updated", "Lead actualizado", { patch });
      await logCrmEvent({ entity_type: "lead", entity_id: id, type: "lead_updated", payload: { patch } });
    }
  }
...
export async function markContactedNow(id: string) {
  ...
  await logCrmEvent({ entity_type: "lead", entity_id: id, type: "lead_contacted" });
  return row;
}
...
export async function setFollowUp(id: string, isoOrNull: string | null) {
  ...
  await logCrmEvent({ entity_type: "lead", entity_id: id, type: "lead_followup", payload: { next_follow_up_at: isoOrNull } });
  return row;
}
