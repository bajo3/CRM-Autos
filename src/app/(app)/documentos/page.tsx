...
import { FileText, ExternalLink, Calculator, ClipboardList, Car, Gauge } from "lucide-react";
...
import { GeneradorDocumentos, type GeneradorItem } from "@/components/documentos/generador-documentos";
import { PreviewDocumento } from "@/components/documentos/preview-documento";
import {
  generarPresupuesto,
  ...
  return (
    <div className="mb-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="h-[70px] animate-pulse rounded-xl border bg-gray-100" />
      ))}
    </div>
  );
}
...
async function NuevosDocumentos() {
  ...
  const items: GeneradorItem[] = [
    {
      id: "presupuesto",
      titulo: "Presupuesto",
      descripcion: "Precio, forma de pago y financiación para un cliente.",
    ...
    },
    {
      id: "datero",
      titulo: "Datero",
      descripcion: "Contacto y vehículo de interés a partir de un cliente ya cargado.",
    ...
    {
      id: "autorizacion_conducir",
      titulo: "Autorización para conducir",
      descripcion: "Autorizá a un tercero a circular con un vehículo de stock.",
    ...
    {
      id: "autorizacion_test_drive",
      titulo: "Autorización de prueba de manejo",
      descripcion: "Prueba de manejo del vehículo en el local.",
  ...
  return <GeneradorDocumentos items={items} />;
}
                      ...
                      <TD>
                        {d.pdf_url ? (
                          <div className="flex items-center gap-2">
                            <PreviewDocumento
                              documentoId={d.id}
                              titulo={`${TIPO_LABEL[d.tipo] ?? d.tipo}${d.numero ? ` ${d.numero}` : ""}`}
                            />
                            <Link
                              href={`/documentos/${d.id}/abrir`}
                              target="_blank"
