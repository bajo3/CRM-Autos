...
export default function TasksPage() {
  ...
  const [view, setView] = useState<"all" | "today" | "overdue" | "week" | "done">("today");
  const [assignedTo, setAssignedTo] = useState<"all" | "team" | string>("all");
  const [search, setSearch] = useState("");

  function applyView(v: "all" | "today" | "overdue" | "week" | "done") {
  setActionError(null);
  setView(v);
  if (v === "done") setStatus("done");
  else setStatus("open");
}

  const [actionError, setActionError] = useState<string | null>(null);

  const { items, counts, loading, error, refresh, role: myRole, userId } = useTasks({ status, assignedTo, search, view });
        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                Hoy {counts ? `(${counts.today})` : ""}
              </Button>
              <Button
                variant={view === "overdue" ? "secondary" : "outline"}
                onClick={() => applyView("overdue")}
              >
                Vencidas {counts ? `(${counts.overdue})` : ""}
              </Button>
              <Button
                variant={view === "week" ? "secondary" : "outline"}
                onClick={() => applyView("week")}
              >
                Semana {counts ? `(${counts.week})` : ""}
              </Button>
              <Button
                variant={view === "done" ? "secondary" : "outline"}
                onClick={() => applyView("done")}
              >
                Hechas {counts ? `(${counts.done})` : ""}
              </Button>
              <Button
                variant={view === "all" ? "secondary" : "outline"}
                onClick={() => applyView("all")}
              >
                Todas {counts ? `(${counts.open})` : ""}
              </Button>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-12">
              ...
              <div className={myRole === "admin" ? "md:col-span-9" : "md:col-span-12"}>
