...
export function fmtDate(iso: string | Date | null | undefined) {
  if (!iso) return "—";
  const raw = iso instanceof Date ? null : String(iso);
  const d =
    iso instanceof Date
      ? iso
      : /^([0-9]{4})-([0-9]{2})-([0-9]{2})$/.test(raw)
...
export function computeCreditSchedule(
  ...
  const start = /^([0-9]{4})-([0-9]{2})-([0-9]{2})$/.test(String(startIso))
    ? new Date(String(startIso) + "T00:00:00")
    : new Date(startIso);
