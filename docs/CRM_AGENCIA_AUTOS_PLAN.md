...
- **Hallazgo importante para toda la sesión:** la limitación de "no se puede enviar formularios `useFormState` por click automatizado" (documentada desde el bloque de Test Drive/Permutas y confirmada hasta en el propio `/login`) **tiene una solución**: en vez de `preview_click` sobre el botón, ejecutar `document.querySelector('form').requestSubmit()` vía `preview_eval`. Se probó en `/login` (bloqueado 3 veces seguidas con click, incluso esperando 6s) y funcionó al primer intento con `requestSubmit()` — logueó correctamente y redirigió al dashboard. La causa parece ser que el click del binding de Chrome DevTools Protocol llega antes de que React termine de adjuntar el listener del formulario en la hidratación, mientras que `requestSubmit()` ejecutado un instante después sí encuentra el listener ya adjunto. **Esto destraba la verificación real en navegador de todos los módulos marcados como "confirmar en navegador real" (Test Drive, Permutas) y "probar flujo completo" (Presupuestos, Consignados/venta con consignación, Catálogo)** — queda pendiente re-visitarlos con esta técnica en un bloque futuro del loop.
- **Pruebas hechas:** `npm run typecheck` + `npm run lint` + `npm run build` en verde. Verificación real en navegador con la técnica `requestSubmit()`: login exitoso, navegación a `/presupuestos/nuevo`, `preview_snapshot` confirmó las 4 tarjetas con sus títulos/descripciones y los selects de cliente/vehículo poblados con datos reales de la empresa demo; se rellenó "Precio" con `preview_fill` y `preview_inspect` confirmó que "Saldo a financiar" se recalculó correctamente a "$ 5.000.000" dentro del nuevo layout (sin regresión en la lógica). `preview_screenshot` falló por timeout (limitación ya conocida de la herramienta) pero la verificación por `snapshot`+`inspect` fue suficiente y más precisa.
