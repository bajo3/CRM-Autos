...
- [ ] **2.1 Ruta `/whatsapp`** (+ `loading.tsx`, ítem de nav en sección nueva "WhatsApp" gateado por `whatsapp.ver`): layout 3 paneles (lista chats / chat / ficha cliente). Server component + paginación de conversaciones (`{count:"exact"}`, `.range()`).
- [ ] **2.2 Lista**: buscador (nombre/teléfono), filtros (abiertas/pendientes/cerradas/sin asignar/bot on/off), preview + hora + badge no-leídos.
- [ ] **2.3 Chat**: mensajes paginados, burbujas por dirección, estados (enviado/entregado/leído/fallado), composer con envío manual (server action → service; si fuera de 24 h muestra selector de plantilla en lugar de texto libre). Polling ~5 s en cliente.
- [ ] **2.4 Panel ficha**: datos del cliente, estado lead, vehículo de interés (link a ficha), vendedor asignado, bot on/off, botones: crear cliente (si el matching falló), asignar vendedor, cerrar/reabrir, crear seguimiento (reusa tabla `seguimiento`).
- [ ] **2.5 Control humano**: acciones asignar/tomar/cerrar/reabrir/pausar bot; **regla: mensaje manual de un usuario → `bot_pausado_hasta = now() + intervalo config`** (default 4 h, configurable en Etapa 3).
- [ ] **2.6 Verificación**: flujo completo con webhook simulado → chat visible en bandeja → respuesta manual con `WHATSAPP_FAKE_SEND=1` → estados reflejados; typecheck+lint+build. Commit.

## Etapa 3 — Bot IA limitado al negocio 🤖

- [ ] **3.1 Config por agencia**: fila `whatsapp_bot_config` por empresa (habilitado, nombre comercial, dirección, horarios, financiación, política de permuta, mensaje fallback, keywords de handoff, tono, minutos de pausa por intervención humana). UI en `/whatsapp/configuracion` (permiso `whatsapp.bot`).
- [ ] **3.2 Agente**: `src/lib/whatsapp/bot.ts` — contexto: config + ficha cliente + últimos 10 mensajes + stock real (query `disponible/publicado`, máx 20, con precio). Reglas duras: no inventar autos/precios/financiación; derivar si (intención fuerte de compra, negociación de precio, enojo/reclamo, no sabe). Salida JSON validada. Handoff → conversación `pendiente` + `bot_pausado_hasta` + mensaje fallback al cliente.
- [ ] **3.3 Integración webhook**: mensaje entrante → si bot habilitado (global + conversación) y no pausado → responder; guardar `enviado_por_bot=true`. Al cerrar conversación con participación del bot → resumen como nota/seguimiento en la ficha del cliente.
- [ ] **3.4 Verificación**: conversación simulada de punta a punta (stock/precio/horarios + un caso de handoff por keyword); comportamiento correcto sin `ANTHROPIC_API_KEY` (bot apagado con aviso, módulo funcional); typecheck+lint+build. Commit.
